...
class URLImporter:
    ...
    async def crawl_level(self, urls: List[Tuple[str, int]], start_url: str) -> Tuple[Dict[str, Tuple[str, int]], List[Tuple[str, int]]]:
        """同じ深さのURLを並列でクロール"""
        tasks = []
        task_urls = []
        results = {}
    ...
    async def crawl(self, start_url: str) -> Dict[str, str]:
        ...
        with tqdm(desc="Downloading pages", unit="pages", leave=True) as pbar:
            ...
            while queue:
                ...
                # 現在のレベルを並列処理
                results, new_urls = await self.crawl_level(current_level_urls, start_url)
    ...
    async def import_from_url(self, url: str):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            with tqdm(total=len(pages), desc="Saving files", unit="files") as pbar:
                futures = []
